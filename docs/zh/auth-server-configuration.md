...
- **默认值：** 5
- **说明：** 密码重置验证码验证失败达到此次数后，暂时锁定该用户/IP 的继续尝试。设为 0 则不限制。
...
- **默认值：** 5
- **说明：** 更换邮箱验证码验证失败达到此次数后，暂时锁定该用户/IP 的继续尝试。设为 0 则不限制。

### EMAIL_VERIFICATION_CODE_THRESHOLD
- **默认值：** 5
- **说明：** 邮箱验证码验证失败达到此次数后，对该用户/IP 的后续尝试临时锁定 30 分钟。设为 0 则不限制。
...
- **默认值：** 10
- **说明：** MFA 验证码（邮箱、短信、OTP、免密合计）验证失败达到此次数后，对该用户/IP 的后续尝试临时锁定 30 分钟。设为 0 则不限制。

### AUTH_CODE_VERIFIER_THRESHOLD
- **默认值：** 未设置（已注释）
- **说明：** 针对有效授权码的 `code_verifier` 校验失败达到此次数后，对该用户/IP 的后续尝试临时锁定 30 分钟。未设置或设为 0 则不限制。未知授权码不计入次数。
