...
import { CookieStorage, defaultCookieGetter, defaultCookieSetter } from './storage'
import type { CookieOptions, AuthStorage } from './storage'
...
describe('CookieStorage', () => {
  ...
  describe('constructor', () => {
    ...
    it('should use custom cookie getter and setter', () => {
      ...
      expect(customGetter).toHaveBeenCalledWith('test', expect.any(Object))

      storage.setItem('test', 'value')
      expect(customSetter).toHaveBeenCalledWith('test', 'value', expect.any(Object))
    })
  })
  ...
  describe('browser environment', () => {
    ...
    describe('setItem', () => {
      ...
      it('should encode URI component in cookie value', () => {
        const storage = new CookieStorage()
  ...
  describe('server environment', () => {
    ...
    describe('getItem', () => {
      it('should retrieve cookie from request headers', () => {
        const request = {
          headers: {
            get: vi.fn().mockReturnValue('testKey=testValue; otherKey=otherValue')
          }
        } as any
      ...
      it('should decode URI component in server cookie', () => {
        const request = {
          headers: {
            get: vi.fn().mockReturnValue('encoded=hello%20world%21')
          }
        } as any
        const storage = new CookieStorage({ request })
        expect(storage.getItem('encoded')).toBe('hello world!')
      ...
      it('should return null for non-existent cookie in request', () => {
        const request = {
          headers: {
            get: vi.fn().mockReturnValue('existingKey=value')
          }
        } as any
        const storage = new CookieStorage({ request })
        expect(storage.getItem('nonExistent')).toBeNull()
      ...
      it('should warn when no document or request available', () => {
    ...
    describe('setItem', () => {
      it('should set cookie header on response', () => {
        ...
        storage.setItem('key', 'value')
        expect(response.headers.append).toHaveBeenCalledWith('Set-Cookie', expect.stringContaining('key=value'))
      })

      it('should encode URI component in server cookie', () => {
        const response = {
          headers: {
            append: vi.fn()
          }
        ...
        const storage = new CookieStorage({ response })
        storage.setItem('key', 'hello world!')
        expect(response.headers.append).toHaveBeenCalledWith('Set-Cookie', expect.stringContaining('key=hello%20world!'))
      })

      it('should include HttpOnly flag in server environment', () => {
        const response = {
          headers: {
            append: vi.fn()
          }
        } as any
        const storage = new CookieStorage({ response, httpOnly: true })
        storage.setItem('key', 'value')
        expect(response.headers.append).toHaveBeenCalledWith('Set-Cookie', expect.stringContaining('HttpOnly'))
      })
      ...
      it('should warn when no document or response available', () => {
        const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => { })
        const storage = new CookieStorage()
        storage.setItem('key', 'value')
  ...
  describe('defaultCookieGetter', () => {
    it('should work independently in browser environment', () => {
      globalThis.document = {
        cookie: 'key=value'
      } as any
      const result = defaultCookieGetter('key', {} as CookieOptions)
      expect(result).toBe('value')
    })

    it('should work independently in server environment', () => {
      delete (globalThis as any).document
      ...
      const result = defaultCookieGetter('key', options)
      expect(result).toBe('value')
    })
  })

  describe('defaultCookieSetter', () => {
    it('should work independently in browser environment', () => {
      globalThis.document = {
        cookie: ''
      } as any
      ...
      defaultCookieSetter('key', 'value', options)
      expect(globalThis.document.cookie).toContain('key=value')
    })
  ...
  describe('edge cases', () => {
    ...
    it('should handle multiple cookies with similar names', () => {
      globalThis.document = {
        cookie: 'key=value1; key2=value2; mykey=value3'
      } as any
      const storage = new CookieStorage()
    ...
    it('should handle special characters in cookie names and values', () => {
      globalThis.document = {
        cookie: 'special%2Bname=special%2Bvalue%3D%3D'
      } as any
      const storage = new CookieStorage()
  ...
  describe('AuthStorage interface compliance', () => {
    it('should implement the AuthStorage interface', () => {
      const storage = new CookieStorage()
    ...
    it('should be usable as AuthStorage type', () => {
