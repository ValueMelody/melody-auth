...
vi.mock('tools', () => ({ accessTool: { Access: { ReadLog: 'read_log' }, isAllowedAccess: vi.fn(), }, }),)
vi.mock('i18n/navigation', () => ({ useRouter: () => ({ push: pushMock, }), }),)

describe('Layout component for logs', () => {
  ...
  it('renders children when user has access', () => {
    ...
    expect(accessTool.isAllowedAccess).toHaveBeenCalledWith(accessTool.Access.ReadLog, ['admin'])
  })
  ...
  it('redirects when user does not have access', async () => {
    ...
    expect(accessTool.isAllowedAccess).toHaveBeenCalledWith(accessTool.Access.ReadLog, [])
  })
})