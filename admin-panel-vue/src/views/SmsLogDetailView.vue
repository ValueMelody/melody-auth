...
import { useSmsLog } from '@/api/endpoints/logs'
...
const { data: log, isLoading } = useSmsLog(logId)
</script>
...
<template>
  <div>
    ...
    <template v-else-if="log">
      <Breadcrumb
        :page="'Log #' + log.id"
        :parent="{ href: '/logs?tab=sms', label: t('logs.smsLogs') }"
      />
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow v-if="log.success !== undefined">
          ...
          <TableRow>
            ...
            <TableCell>
              <pre class="text-sm bg-muted p-2 rounded max-h-48 overflow-auto">{{ log.content }}</pre>
            </TableCell>
          </TableRow>
