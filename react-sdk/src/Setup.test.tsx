...
describe('Setup Component', () => {
  ...
  it('should do nothing if accessTokenStorage exists', async () => {
    const state = { ...defaultState, accessTokenStorage: 'existingAccessToken' }
  ...
  it('should call acquireToken if no code in URL and refreshTokenStorage exists', async () => {
    const state = { ...defaultState, refreshTokenStorage: 'existingRefreshToken' }
    const contextValue = { state, dispatch }
    // Set a URL that does NOT contain a 'code' parameter.
    window.history.pushState({}, 'Test Title', '/?someParam=value')
  ...
  it('should call exchangeTokenByAuthCode and dispatch actions on success when code is present', async () => {
    // Set URL with 'code', and additional query params for 'state' and 'locale'
    window.history.pushState({}, 'Test Title', '/?code=sampleCode&state=xyz&locale=en')
    const onLoginSuccess = vi.fn()
    const state = { ...defaultState, config: { onLoginSuccess } }
    const contextValue = { state, dispatch }
    ...
    await waitFor(() => {
      ...
      expect(onLoginSuccess).toHaveBeenCalledWith({ state: 'xyz', locale: 'en' })
    })
  })

  it('should dispatch setIsAuthenticating if exchangeTokenByAuthCode returns without accessTokenStorage', async () => {
    window.history.pushState({}, 'Test Title', '/?code=sampleCode')
  ...
  it('should dispatch authentication error if exchangeTokenByAuthCode fails', async () => {
    window.history.pushState({}, 'Test Title', '/?code=sampleCode')
    const error = new Error('Token exchange failed')

    // Mock handleError to return a predictable error message.
    vi.spyOn(utils, 'handleError').mockReturnValue('handled error message')
  ...
  it('should do nothing on subsequent effect invocations if already initialized', async () => {
    ...
    const initialState = { ...defaultState, checkedStorage: true }
    const contextValue = { state: initialState, dispatch }
    ...
    const newState = { ...initialState, checkedStorage: false }
    const newContextValue = { state: newState, dispatch }
    rerender(
