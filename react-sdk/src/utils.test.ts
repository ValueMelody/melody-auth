...
import { ErrorType, handleError } from './utils'

describe('handleError', () => {
  it('should return Unauthorized if error message includes "Unauthorized"', () => {
    const error = new Error('Unauthorized access detected')
    const result = handleError(error)
    expect(result).toBe(ErrorType.Unauthorized)
  })

  it('should return the provided fallback when error does not include "Unauthorized"', () => {
  ...
  it('should return Unknown if no fallback is provided and error does not include "Unauthorized"', () => {
