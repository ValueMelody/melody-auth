...
const AuthorizePassword = ({ queryDto }: {
  ...
  return (
    <Layout>
      ...
      `}
      <form id="password-signin-form" onsubmit="return handleSubmit()">
        <section class="flex-col gap-4">
          <section class="flex-col gap-2">
            <label class="label" for="email">{localeConfig.AuthorizePasswordPage.EmailLabel}</label>
            <input class="input" type="email" id="form-email" name="email" />
          </section>
          <section class="flex-col gap-2">
            <label class="label" for="password">{localeConfig.AuthorizePasswordPage.PasswordLabel}</label>
          ...
          <input type="hidden" id="form-code-challenge" name="codeChallenge" value={queryDto.codeChallenge} />
          <input
            type="hidden"
