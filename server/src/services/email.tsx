...
import { errorConfig, localeConfig, typeConfig } from "configs";
import { emailLogModel, userModel } from "models";
import {
...
export const sendEmail = async (
    ...
    if (c.env.SMTP) {
    ...
    } else if (postmarkApiKey && postmarkSender) {
        const res = await fetch("https://api.postmarkapp.com/email", {
            ...
            body: JSON.stringify({
                From: "Melody Auth <" + postmarkSender + ">",
                To: receiver,
                Subject: subject,
                HtmlBody: emailBody,
    ...
    if (enableEmailLog) {
        await emailLogModel.create(c.env.DB, {
            success: success ? 1 : 0,
            receiver,
...
export const sendEmailVerification = async (
    ...
    const { COMPANY_LOGO_URL: logoUrl, AUTH_SERVER_URL: serverUrl } = env(c);
...
export const sendEmailMfa = async (
    ...
    const content = (
        <EmailMfaTemplate mfaCode={mfaCode} logoUrl={logoUrl} locale={locale} />
    ).toString();
