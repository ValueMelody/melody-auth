...
import { localeConfig, routeConfig, typeConfig } from 'configs'
import * as requestModule from 'pages/tools/request' // used for handleAuthorizeStep
...
const dummyParams = {} // dummy value for params
...
test('handleSubmit blocks submission when validation errors exist', async () => {
  ...
  expect(result.current.errors.lastName).toEqual(
    localeConfig.validateError.lastNameIsEmpty.en
  )

  fetchSpy.mockRestore()
})

test('handleSubmit submits form successfully when valid input is provided', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  ...
  const fakeResponse = { ok: true, json: async () => ({ status: 'ok' }) }
...
test('handleSubmit calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const fakeError = new Error('Network error')
  const fetchSpy = vi.spyOn(global, 'fetch').mockRejectedValue(fakeError)
