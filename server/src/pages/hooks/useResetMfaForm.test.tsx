...
test(
  'returns initial state with success false and proper redirectUri',
  () => {
    const onSubmitError = vi.fn()
    const { result } = renderHook(() => useResetMfaForm({
...
test(
  'handleSubmit sets success to true on successful fetch',
  async () => {
    const onSubmitError = vi.fn()
    const { result } = renderHook(() => useResetMfaForm({
      locale: 'en', onSubmitError,
