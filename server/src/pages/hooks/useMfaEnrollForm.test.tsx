...
test('returns initial state with null mfaEnrollInfo', () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useMfaEnrollForm({ locale: 'en', onSubmitError, onSwitchView })
  )

  expect(result.current.mfaEnrollInfo).toBeNull()
})

test('getMfaEnrollInfo successfully fetches and updates mfaEnrollInfo', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
...
test('getMfaEnrollInfo calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
...
test('handleEnroll submits enrollment and calls handleAuthorizeStep for success response', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  // Dummy MFA type for testing.
  const dummyMfaType = 'dummy-mfa'
  ...
  await act(async () => {
    result.current.handleEnroll(dummyMfaType)
    await Promise.resolve()
  })
...
test('handleEnroll calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const dummyMfaType = 'dummy-mfa'
