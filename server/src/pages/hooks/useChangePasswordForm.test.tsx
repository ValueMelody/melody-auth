...
test('handleChange updates password and confirmPassword', () => {
  const onSubmitError = vi.fn()
  const { result } = renderHook(() =>
    useChangePasswordForm({
      locale: 'en',
  ...
  act(() => {
    result.current.handleChange('password', 'mySecret')
    result.current.handleChange('confirmPassword', 'mySecret')
  })
...
test('submits form successfully when valid input is provided', async () => {
  const onSubmitError = vi.fn()
...
test('shows validation errors when passwords do not match', async () => {
  ...
  act(() => {
    result.current.handleChange('password', 'Password1!')
    result.current.handleChange('confirmPassword', 'Password2!')
  })
...
test('shows validation errors when password is weak', async () => {
  const onSubmitError = vi.fn()
  const { result } = renderHook(() =>
    useChangePasswordForm({
      locale: 'en',
  ...
  act(() => {
    result.current.handleChange('password', 'abc123')
    result.current.handleChange('confirmPassword', 'abc123')
  })
