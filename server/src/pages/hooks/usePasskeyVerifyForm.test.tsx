...
test('returns initial state with null passkeyOption', () => {
...
test('getPasskeyOption calls onSubmitError if email validation fails', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
...
test('getPasskeyOption sets passkeyOption when response contains passkeyOption', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    usePasskeyVerifyForm({
  ...
  const fakePasskeyOption = { challenge: 'test-challenge', allowCredentials: [{ id: 'test-id' }] }
  const fakeResponse = { ok: true, json: async () => ({ passkeyOption: fakePasskeyOption }) }
...
test('getPasskeyOption sets passkeyOption to false when response has no passkeyOption', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    usePasskeyVerifyForm({
...
test('getPasskeyOption calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    usePasskeyVerifyForm({
...
test('handleVerifyPasskey does nothing when passkeyOption is falsy', () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    usePasskeyVerifyForm({
...
test('handleVerifyPasskey submits passkey when credentials.get succeeds', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  ...
  ;(window as any).SimpleWebAuthnBrowser = {
    base64URLStringToBuffer: (str: string) => str,
  }

  const fakePasskeyOption = { challenge: 'test-challenge', allowCredentials: [{ id: 'test-id' }] }
  // Simulate GET response in getPasskeyOption.
  const fakeGetResponse = { ok: true, json: async () => ({ passkeyOption: fakePasskeyOption }) }
  const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValueOnce(fakeGetResponse as Response)
  ...
  const fakePostResponse = { ok: true, json: async () => ({}) }
  ...
  const handleAuthorizeSpy = vi
    .spyOn(requestModule, 'handleAuthorizeStep')
    .mockImplementation((response, locale, onSwitchView) => {
  ...
  expect(navigatorSpy).toHaveBeenCalledWith({
    publicKey: {
      ...
      allowCredentials: [{ id: fakePasskeyOption.allowCredentials[0].id, type: 'public-key' }],
    },
  })
...
test('handleVerifyPasskey calls onSubmitError when navigator.credentials.get fails', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    usePasskeyVerifyForm({
...
test('submitPasskey (via handleVerifyPasskey) calls onSubmitError when POST fetch fails', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const params = {} as any
