import {
  describe, test, vi, expect, afterEach,
  Mock,
} from 'vitest'
...
describe(
  ...
  // Reset mocks and window location between tests.
    ...
    test(
      'returns view from getStepFromParams if available',
      () => {
        // Force getStepFromParams to return a specific view.
        (getStepFromParams as unknown as Mock).mockReturnValue(View.OtpMfa)
    ...
    test(
      'returns AuthCodeExpired view based on pathname when no step from params',
      () => {
    ...
    test(
      'returns SignIn view as default when no matching pathname and no step from params',
      () => {
        (getStepFromParams as unknown as Mock).mockReturnValue(null)
