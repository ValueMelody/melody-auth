...
test('initial state returns null consentInfo', () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useConsentForm({ locale: 'en', onSubmitError, onSwitchView })
  )
...
test('getConsentInfo fetches consent info and updates state', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useConsentForm({ locale: 'en', onSubmitError, onSwitchView })
  ...
  const fakeResponse = { ok: true, json: async () => fakeConsent }
...
test('getConsentInfo calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useConsentForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('handleAccept calls onSubmitError on POST failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useConsentForm({ locale: 'en', onSubmitError, onSwitchView })
  ...
  const fetchSpy = vi.spyOn(global, 'fetch').mockRejectedValue(new Error('POST error'))
