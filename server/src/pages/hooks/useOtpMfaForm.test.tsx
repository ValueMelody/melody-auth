...
vi.mock('pages/tools/param', () => ({
  getFollowUpParams: vi.fn(() => ({ code: 'test-code', org: 'test-org' })),
}))
...
test('returns initial state', () => {
  ...
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
  )
...
test('getOtpSetupInfo fetches OTP setup info and updates otpUri', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('getOtpSetupInfo calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('getOtpMfaInfo fetches OTP MFA info and updates allowFallbackToEmailMfa', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('getOtpMfaInfo calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('handleMfa submits data successfully when valid', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('handleMfa does not submit if validation errors exist', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
...
test('handleMfa calls onSubmitError on fetch failure', async () => {
  const onSubmitError = vi.fn()
  const onSwitchView = vi.fn()
  const { result } = renderHook(() =>
    useOtpMfaForm({ locale: 'en', onSubmitError, onSwitchView })
