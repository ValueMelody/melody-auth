...
test('handleChange updates email and mfaCode', () => {
  const onSubmitError = vi.fn()
  const { result } = renderHook(() =>
    useChangeEmailForm({
      locale: 'en',
...
test('submits code when mfaCode is null (send code branch)', async () => {
  const onSubmitError = vi.fn()
...
test('submits form successfully when mfaCode is provided (change email branch)', async () => {
  const onSubmitError = vi.fn()
  const fakeResponse = { ok: true, json: async () => ({}) }
  const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue(fakeResponse as Response)
...
test('handleResend sends code and updates resent state', async () => {
  const onSubmitError = vi.fn()
  const fakeResponse = { ok: true, json: async () => ({}) }
  const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue(fakeResponse as Response)
...
test('calls onSubmitError when send code submission fails', async () => {
  const onSubmitError = vi.fn()
...
test('calls onSubmitError when change email submission fails', async () => {
  const onSubmitError = vi.fn()
